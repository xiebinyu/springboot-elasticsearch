...
public class TestController {
    ...
    public String test2(){
        ...
        SearchResponse sr = client.prepareSearch("project_study").setTypes("article") //要查询的表
                .setQuery(QueryBuilders.boolQuery()
                ).addAggregation(
                        AggregationBuilders.terms("type").field("type").size(500)) //按货号分组，最多查500个货号.SKU直接改字段名字就可以

                .execute().actionGet();

        Terms terms = sr.getAggregations().get("type");   //查询遍历第一个根据货号分组的aggregation

        System.out.println(terms.getBuckets().size());
        for (Terms.Bucket entry : terms.getBuckets()) {
            System.out.println("------------------");
            System.out.println("【 " + entry.getKey() + " 】文章数 : " + entry.getDocCount() );
